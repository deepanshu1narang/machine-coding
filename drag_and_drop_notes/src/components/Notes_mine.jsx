...
const Notes = ({ notes = [], setNotes }) => {
  ...
  return (
    <div className="notes-area">
      {notes.map((note) => {
        return <Note key={note.id} ref={noteRefs.current[note.id] ? noteRefs.current[note.id] : (noteRefs.current[note.id] = createRef())} note={note} position={note.position} onMouseDown={e => handleDragStart(note, e)} />;
      })}
    </div>
  );
};
